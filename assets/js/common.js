...
//main aos 아이템 순차적으로 나타나게
function setAOSDelays() {
    const rowDelay = 100;
    const colDelay = 200;

    document.querySelectorAll(".thumnail").forEach((ul) => {
        ...
        Object.values(rows).forEach((row, rowIndex) => {
            row.forEach((li, colIndex) => {
                const delay = rowIndex * rowDelay + colIndex * colDelay;
...
window.addEventListener("resize", setAOSDelays);
